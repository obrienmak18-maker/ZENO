...
export default function EntryScreen() {
  ...
  const handleScan = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      ...
      if (functions && auth) {
        const generate = httpsCallable(functions, 'generateQRAuthToken');
        const result = await generate({ tokenId: payload.tokenId, schoolId: payload.schoolId });
        const customToken = (result.data as { customToken?: string }).customToken;
        if (!customToken) throw new Error('Custom Token absent');
        await signInWithCustomToken(auth, customToken);
      } else if (firebaseConfigured) {
        throw new Error('Firebase mobile non initialisé');
      }
