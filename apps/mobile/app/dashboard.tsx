...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    if (user) void user.getIdTokenResult().then((token) => { const nextRole = String(token.claims.role ?? 'ADMINISTRATEUR'); setRole(nextRole); if (nextRole === 'PROFESSEUR') router.replace('/(prof)/(tabs)' as never); if (nextRole === 'COMPTABLE') router.replace('/(comptable)/(tabs)' as never); });
  }, []);
  useEffect(() => {
    const user = auth?.currentUser;
